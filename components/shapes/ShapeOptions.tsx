...
const strokeColors = ["white", "red", "black", "blue", "green"];
const backgroundColors = [...strokeColors, "transparent"];
const cornerRadius = [0, 100];

export const ShapeOptions: FC<ShapeOptionsProps> = ({
  onApplyStyles,
  activeShapes,
  style,
  deleteShapes,
      ...
      <div className="flex flex-col gap-5">
        {options.map((option) => (
          <div key={option.title} className="flex flex-col items-start">
            {option.key === "cornerRadius" ? (
              <Slider
                label={option.title}
                onChange={(value) => onApplyStyles({ [option.key]: value })}
                step={1}
                maxValue={option.options[1] as number}
                minValue={option.options[0] as number}
                defaultValue={5}
                className="max-w-md"
              />
            ) : (
